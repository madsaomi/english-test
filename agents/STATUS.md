...
**Последнее обновление:** 2026-09-22 01:10 (UTC+5)  
**Ответственный агент:** opencode (opencode/big-pickle)  
**Текущая фаза:** Минималистичный интерфейс (PLAN-005) — на сайте без подсказок: только вопросы, форма контактов и «Спасибо! Подойдите к администратору»
...
| **Веб-интерфейс (`static/`)** | 🟢 Готово | Минималистичный Glassmorphism UI; на экране вопроса только счётчик/таймер/прогресс/вопрос/варианты (без уровня/категории/темы); каталог чистый; хоткеи через `e.code` |
...
- Завершён PLAN-005 (TASK-007): убраны все подсказки (название теста, уровень, категория, тема) и маркетинговый шум; интерфейс минималистичный. Бэкенд не менялся, CPT: HTML+JS+CSS.
- Напомню: за работой PLAN-003/004 результат на сайте не показывается — сотрудник получает карточку + детальный результат в Telegram. Для реальной отправки нужны `BOT_TOKEN` и `ADMIN_CHAT_ID` в `.env`.
- Следующие кандидаты: вычистка мёртвых CSS-классов (`.q-level-badge`, `.category-tag`, `.topic-pill`, `.features-grid` и т.п.), PDF/Canvas-сертификат, интерактивная карта уровней CEFR, история попыток по `tg_user_id`, настройка прод-доменов в `ALLOWED_ORIGINS`.