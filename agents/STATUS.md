...
**Последнее обновление:** 2026-09-22 01:45 (UTC+5)  
**Ответственный агент:** opencode (opencode/big-pickle)  
**Текущая фаза:** Бумажный светлый стиль (PLAN-006) — тёплая бумага, frosted-карточки, чернильный текст; подсказки убраны, результат на сайте не показывается
...
| **Веб-интерфейс (`static/`)** | 🟢 Готово | Бумажный светлый стиль: фон `#f3f1ec` + фактура, frosted-карточки, чернильный текст `#1c1917`; без подсказок; хоткеи через `e.code` |
...
- Завершён PLAN-006 (TASK-008): интерфейс переведён на бумажный светлый стиль (фон-бумага + фактура, frosted-glass карточки, чернильные тексты/рамки, тёплые тени, затемнённые акценты); ambient-свечения удалены. CPT: HTML+CSS, бэкенд и `app.js` не менялись.
...
- Следующие кандидаты: вычистка мёртвых CSS-классов (`.q-level-badge`, `.category-tag`, `.features-grid`, `.review-*`, `.skill-*` и т.п.), PDF/Canvas-сертификат, интерактивная карта уровней CEFR, история попыток по `tg_user_id`, настройка прод-доменов в `ALLOWED_ORIGINS`.