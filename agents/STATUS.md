...
**Текущая фаза:** Основной тест General English Test 2026 (PLAN-010) — 50 вопросов (45 choice + 5 text), старые автотесты удалены, поле ввода с case-sensitive проверкой
...
- Завершён PLAN-010 (TASK-012): `test_general_2026` — единственный основной тест (50 вопросов, 15 мин); удалены 3 JSON + `cefr_adaptive` + `QUESTION_BANK` + `cefr_adaptive_bank`; 5 fill-in — поле ввода case-sensitive; API 400-валидация; тесты и user-facing копирайт переписаны. Изменения НЕ закоммичены — ждать «да».
